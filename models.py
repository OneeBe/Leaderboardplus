...
class Player(db.Model):
    ...
    def effective_role_data(self):
        """Get complete role data for display with priority"""
        # Priority 1: Active admin role
        if self.active_admin_role:
            admin_role = self.active_admin_role
            return {
                'type': 'admin',
                'name': admin_role.role.name,
                'color': admin_role.role.color,
                'gradient': admin_role.role.gradient if hasattr(admin_role.role, 'gradient') else None,
                'gradient_start': admin_role.role.gradient_start if hasattr(admin_role.role, 'gradient_start') else None,
                'gradient_end': admin_role.role.gradient_end if hasattr(admin_role.role, 'gradient_end') else None,
                'gradient_animated': admin_role.role.gradient_animated if hasattr(admin_role.role, 'gradient_animated') else False,
                'emoji': admin_role.role.emoji,
                'has_gradient': bool(getattr(admin_role.role, 'has_gradient', False))
            }
        # Priority 2: Purchased custom role
        elif getattr(self, 'custom_role_purchased', False) and self.custom_role:
            return {
                'type': 'custom',
                'name': self.custom_role,
                'color': self.custom_role_color,
                'gradient': self.custom_role_gradient,
                'gradient_animated': self.custom_role_animated,
                'emoji': self.custom_role_emoji,
                'has_gradient': bool(self.custom_role_gradient)
            }
        # Priority 3: Regular role with possible gradient
        elif self.role_gradient:
            return {
                'type': 'default_gradient',
                'name': self.role,
                'gradient': self.role_gradient,
                'has_gradient': True
            }
        # Priority 4: Basic role
        else:
            return {
                'type': 'default',
                'name': self.role,
...
class GameMode(db.Model):
    ...
    def create_default_modes(cls):
        """Create default game modes - Only Bedwars"""
        default_modes = [
            {
                'name': 'bedwars',
        ...
        # Remove all existing modes except Bedwars
        existing_modes = cls.query.filter(cls.name != 'bedwars').all()
